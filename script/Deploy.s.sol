...
contract Deploy is Script {
    ...
    function _logDeployment(Deployment memory d, IVRFCoordinatorV2 coordinator) internal view {
        ...
        console.log("GAME_TOKEN=%s", address(gameToken));
        console.log("GOLD=%s", address(gold));
        console.log("CRYSTAL=%s", address(crystal));
        console.log("ITEMS=%s", address(items));
        console.log("CRAFTING=%s", address(crafting));
        console.log("LOOT_DROP=%s", address(lootDrop));
        console.log("RENTAL_VAULT=%s", address(rentalVault));
        console.log("VAULT=%s", address(vault));
        console.log("AMM_POOL=%s", address(pool));
        console.log("GOVERNOR=%s", address(governor));

        console.log("EXPORT_END");
    }
}
